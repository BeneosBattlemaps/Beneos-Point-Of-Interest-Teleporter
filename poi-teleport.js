...
class PointOfInterestTeleporter {
  ...
  static onReady() {
    ...
    canvas.mouseInteractionManager.target.on("rightdown", () => canvas.hud.poiTp.close());
    canvas.mouseInteractionManager.target.on("mousedown", () => canvas.hud.poiTp.close());
  ...
  static renderHeadsUpDisplay(hud, html) {
    console.log("renderHeadsUpDisplay", hud, html);
  ...
  static async checkNote(note) {
    // Deprecated : const scene = game.scenes.find(s => s.journal?.id == note?.entry?.id);
    let scene;
...
class PoiTpHUD extends HandlebarsApplicationMixin(foundry.applications.hud.BasePlaceableHUD) {
  ...
  static executeOption(event, target) {
    ...
    console.log("executeOption", event, target, trigger);
  ...
  */
   ...
   */
  setPosition() {
    console.log("setPosition", this.object);
    const position = {
      left: this.object.x,
      top: this.object.y,
      "font-size": this.poitp.note.document.iconSize / 2 + "px" // Fix #2
    };
